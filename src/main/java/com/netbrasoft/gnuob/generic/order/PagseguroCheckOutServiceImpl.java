...
public class PagseguroCheckOutServiceImpl<O extends Order> implements CheckOutService<O> {
    ...
    public PagseguroCheckOutServiceImpl() {
        if (!PAGSEGURO_PRODUCTION_TOKEN_PROPERTY_VALUE.equals(System.getProperty(PAGSEGURO_PRODUCTION_TOKEN_PROPERTY, PAGSEGURO_PRODUCTION_TOKEN_PROPERTY_VALUE))) {
    ...
    private String createCheckoutRequest(Checkout checkout) throws PagSeguroServiceException, ParserConfigurationException, SAXException, IOException {
        String email = System.getProperty(PAGSEGURO_EMAIL_PROPERTY, PAGSEGURO_EMAIL_PROPERTY_VALUE);
        String productionToken = System.getProperty(PAGSEGURO_PRODUCTION_TOKEN_PROPERTY, PAGSEGURO_PRODUCTION_TOKEN_PROPERTY_VALUE);
        String sandboxToken = System.getProperty(PAGSEGURO_SANDBOX_TOKEN_PROPERTY, PAGSEGURO_SANDBOX_TOKEN_PROPERTY_VALUE);
    ...
    public void doCheckout(O order) {
        ...
        checkout.setCurrency(Currency.valueOf(System.getProperty(PAGSEGURO_CURRENCY_PROPERTY, PAGSEGURO_CURRENCY_PROPERTY_VALUE)));
        checkout.setExtraAmount(order.getExtraAmount().setScale(2));
        checkout.setNotificationURL(System.getProperty(GNUOB_SITE_PROPERTY, GNUOB_SITE_PROPERTY_VALUE));
        checkout.setRedirectURL(System.getProperty(GNUOB_SITE_PROPERTY, GNUOB_SITE_PROPERTY_VALUE));

      ...
      for (Object parameter : ctx.getParameters()) {
         ...
         if (parameter instanceof MetaData) {
            ...
            if (siteServiceImpl.count((S) new Site(metaData.getSite())) > 0) {
               subject.site = siteServiceImpl.find((S) new Site(metaData.getSite()), Paging.getInstance(-1, -1),
                     OrderBy.NONE).get(0);
            } else {
               throw new GNUOpenBusinessServiceException(String.format(
                     "Given site [%s] doesn't have the right access, verify that the given site has access",
                     metaData.getSite()));
            }

            // Check user credentials
            if (userServiceImpl.count((U) new User(metaData.getUser(), metaData.getPassword())) > 0) {
               subject.user = userServiceImpl.find((U) new User(metaData.getUser(), metaData.getPassword()),
                     Paging.getInstance(-1, -1), OrderBy.NONE).get(0);
            } else {
               throw new GNUOpenBusinessServiceException(String.format(
                     "Given user [%s] doesn't have the right access, verify that the given user has access",
                     metaData.getUser()));
            }
      ...
      for (Object parameter : ctx.getParameters()) {
         ...
         if (parameter instanceof Access) {
            ...
            if (access != null) {
               ...
               // access?
               if (access.getOwner() != null && access.getOwner().equals(subject.user)

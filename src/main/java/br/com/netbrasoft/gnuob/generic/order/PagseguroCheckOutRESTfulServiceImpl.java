...
import static br.com.netbrasoft.gnuob.generic.NetbrasoftSoapConstants.PAGSEGURO_CHECK_OUT_RESTFUL_SERVICE_IMPL;
...
@Path(PAGSEGURO_CHECK_OUT_RESTFUL_SERVICE_IMPL)
@Consumes({APPLICATION_JSON})
@Produces({APPLICATION_JSON})
@Interceptors(value = {AppSimonInterceptor.class, MailControl.class})
public class PagseguroCheckOutRESTfulServiceImpl implements ICheckOutWebService<Order> {

  private static final Logger LOGGER = getLogger(PagseguroCheckOutRESTfulServiceImpl.class);
  ...
  public PagseguroCheckOutRESTfulServiceImpl() {
    // This constructor will be used by the EJB container.
  }

  PagseguroCheckOutRESTfulServiceImpl(final ISecuredGenericTypeCheckOutService<Order> securedGenericTypeCheckOutService,
  ...
  private Order processNotification(final MetaData credentials, final Order type) {
    final Order notified = securedGenericTypeCheckOutService.doNotification(credentials, type);
